/**
 * Stock data fetching utilities for the React app
 */
...
/**
 * Raw API response types (before conversion to typed interfaces)
 */
export interface RawStockHistoryItem {
  date: string | number;
  open: number;
  high: number;
  low: number;
...
export interface RawMomentumData {
  Date: string | number;
  Close: number;
  RSI_14: number | null;
  MOM_10: number | null;
...
export interface RawSignalsData {
  Date: string | number;
  Close: number;
  rsi: number | null;
...
/**
 * Downsample stock history data for chart rendering
 */
 ...
 * @param limit - Optional limit for downsampling (default 252 points for ~1 year of trading days)
 */
export async function fetchHistory(
  ...
  // Apply downsampling if limit is provided and data exceeds limit
...
/**
 * Fetch stock info/company details
 */
 ...
 */
export async function fetchHistoryMetadata(symbol: string): Promise<HistoricalMetadata> {
  ...
  if (json.chart.error) {
    throw new Error(json.chart.error.description || "Failed to fetch metadata");
  }
  ...
  return {
    ...
    period: meta.dataGranularity || "1d",
    intervals: String(timestamps.length),
  };
}
...
/**
 * Fetch growth estimates for a stock
 */
...
// Helper to map Python field names to JS
...
// Helper to map signals data from API
...
/**
 * Fetch buy/sell/hold forecast based on technical indicators
 */
...
/**
 * Fetch trading signals based on technical indicators
 */
export async function fetchSignals(symbol: string, period: string = "6mo"): Promise<SignalsData[]> {
  ...
  return result.data.map((d: RawSignalsData) => ({
    date: new Date(d.Date),
    close: d.Close,
    rsi: d.rsi,
    rsi_signal: d.rsi_signal,
...
/**
 * Fetch momentum indicators for a stock
 */
...
/**
 * Cached data fetching using background queue
 * These functions use the /api/queue endpoint which caches results in Redis
 */
